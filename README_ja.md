...
funcityは関数型言語処理系に、テキスト整形のための「手続き的な」構文を加えたスクリプト構文を採用しています。

以下の解説では構文を掘り下げて、あなたがfuncityスクリプトを書けるようにします。
funcityの構文は非常に単純なため、すぐに習得できるでしょう。
...
```funcity
イリオモテヤマネコは{{'西表島'}}に住んでいる。
```

これをfuncityで実行します。すると、以下の出力が得られます:

```bash
$ echo "イリオモテヤマネコは{{'西表島'}}に住んでいる。" | funcity run
イリオモテヤマネコは西表島に住んでいる。
```
...
```bash
$ echo "イリオモテヤマネコは{{1965}}年に発見された。" | funcity run
イリオモテヤマネコは1965年に発見された。
```

数値が指定できるということは、計算も可能です:

```bash
$ echo "イリオモテヤマネコは{{add 1960 5}}年に発見された。" | funcity run
イリオモテヤマネコは1965年に発見された。
```

この`add`とは、funcity標準関数の一つです。詳しくは標準関数を参照してください。
JavaScriptの`Math`オブジェクトを使用することも出来ます:

```bash
$ echo "辺が1cmの正方形の対角線の長さは{{Math.sqrt 2}}cmである。" | funcity run
辺が1cmの正方形の対角線の長さは1.4142135623730951cmである。
```

関数の呼び出しは数値だけではなく、任意の値が使用できます。以下のように文字列を指定することも出来ます:

```bash
$ echo "イリオモテとヤマネコだから、{{concat 'イリオモテ' 'ヤマネコ'}}だ。" | funcity run
イリオモテとヤマネコだから、イリオモテヤマネコだ。
```

関数には複数の引数を指定できる場合があります。引数はスペースで区切って指定するので、ネストした式は曖昧にならないように括弧を指定します:
...
funcityには、やや汚い「手続き的な」な構文があります。
しかし、これを見れば、他のテキストプロセッサと同様の処理ができると、すぐに理解できるでしょう:

```bash
$ echo "イリオモテヤマネコは、{{if true}}猫{{else}}犬{{end}}である。" | funcity run
イリオモテヤマネコは、猫である。
```
...
```bash
$ echo "イリオモテヤマネコ群に、{{for i (range 1 5)}}「猫{{i}}」{{end}}という名前をつけた。" | funcity run
イリオモテヤマネコ群に、「猫1」「猫2」「猫3」「猫4」「猫5」という名前をつけた。
```

この`range`関数は、1から5までの数値のリストを返します。`for`はこのリストの値を一つづつ`i`という変数に入れて、`end`までを繰り返します。

この`for`の内側で、更に`if`を使ってみます:

```bash
$ echo "奇数のイリオモテヤマネコ群{{for i (range 1 5)}}{{if (mod i 2)}}「猫{{i}}」{{end}}{{end}}に、ちゅーるを与えた。" | funcity run
奇数のイリオモテヤマネコ群「猫1」「猫3」「猫5」に、ちゅーるを与えた。
```

注意: 本物のイリオモテヤマネコに、ちゅーるを与えてはいけません。

このように、`for`や`if`を入れ子（ネスト）させることも出来ます。

functiyは任意の名前で変数を定義できます:

```funcity
円周率は長いので、piという変数を定義する。
{{
set pi 3.14159265
}}
そうすれば、piだけで参照できる。半径10cmの円周の長さは{{mul 2 pi 10}}cmだ。
```

このように、複数行に渡ってコードを記述する場合は、任意のファイル（例えば `sample.fc`）に保存しておいて、funcityに流し込みます:

```bash
$ funcity run -i sample.fc
円周率は長いので、piという変数を定義する。

そうすれば、piだけで参照できる。半径10cmの円周の長さは62.831853cmだ。
```

波括弧を閉じた時点の改行が出力されてしまうので、上記のように空行が入る場合があります。これが問題になる場合は、改行位置について注意してください。

MEMO: 変数は後から`set`で上書きできます。専門用語では「ミュータブル」"mutable"と呼びますが、これも関数型言語ではあまり好まれない扱いです。
...
改行を適切に行うために行が長くなっていますが、これを実行すると以下のように出力されます:
...
set abort true
while abort
set eat (readline 'バーベキューを食べますか? [y/n]:')
...
`readline` は、引数の文字列をプロンプトとして出力してから、入力を待ちます。
得られた入力文字列が返され、それが `eat` 変数に代入されます。

`abort` には中断するフラグが格納されていて、`readline`で得られた入力に応じて、「バーベキューを食べたら抜ける」という動作を実現しています。

ここでは入力を、単純な 'y' や 'Y' で比較していますが、正規表現関数 `match` を使えば、もっと柔軟な判定が可能です。
...
```bash
$ echo "引用: {{'I\\'m \\\\ ok'}}" | funcity run
引用: I'm \ ok
```

### リストとイテレーション

funcityでは、`[...]` でリスト（配列）を定義します。要素はスペース区切りです。
`for` の第2引数にリストを渡すと、各要素を取り出して繰り返し処理ができます:

```bash
$ echo "イリオモテヤマネコの番号: {{for i [1 2 3 4 5]}}「猫{{i}}」{{end}}" | funcity run
イリオモテヤマネコの番号: 「猫1」「猫2」「猫3」「猫4」「猫5」
```

`map` や `filter` を使うと、リストを加工して新しいリストを返せます。
`map` は各要素に関数を適用し、`filter` は条件に合う要素だけを残します:

```bash
$ echo "10倍: {{map (fun [x] (mul x 10)) [1 2 3 4]}}" | funcity run
10倍: [10 20 30 40]
```

```bash
$ echo "奇数だけ: {{filter (fun [x] (mod x 2)) [1 2 3 4 5]}}" | funcity run
奇数だけ: [1 3 5]
```

このような関数を受け取る関数（高階関数）は、 `flatMap` や `reduce` など、他にもいくつか定義されています。
前節の `range` や、 `first`, `last`, `at`, `sort`, `collect`, `reverse` などと組み合わせれば、集合演算を簡単に実装できます。

### 式の区切りとコメント

複数の式は、次の3つの方法で区切ることが出来ます:
...
MEMO: `;` は式の区切りとして扱われますが、リスト (`[...]`) の中では使えません。

逆に、式の途中で改行したい場合は、 `\` を行末に記述します:
...
コメントは `//` で始まり、行末まで続くとみなします:

```funcity
{{
// 表示文字列を保存する
set label (concat name 'はクール')
}}
```

### 関数型言語

ここまでの解説で、大半のテキスト整形の目的は十分果たせると考えられますが、ここからはfuncityの関数型言語処理系としての構文を解説します。

関数型言語処理系として、ラムダ式による匿名関数が定義できます:

```bash
$ echo "数値を2倍にする関数を定義する: {{fun x (add x x)}}" | funcity run
...
数値を2倍にする関数を定義して実行する: 42
```

このように、インラインで式を記述すると、一般的な開発者にはあまり馴染みがない式と思われます。
そこで、匿名関数を変数に代入（束縛）してから使用する例を示します:

```funcity
{{
set mul2 (fun x (add x x))
}}
数値を2倍にする関数を定義して実行する: {{mul2 21}}
```

このようにして、カスタムの関数を定義することが出来ます。
...
楕円の面積を求める: {{ellipse 2 3}}
```

```bash
$ funcity -i sample.fc

楕円の面積を求める: 18.84955592153876
```

引数のリストは `[...]` で定義します。上記ではJavaScriptの`Math.PI`を使用して、`πab`の楕円面積の式を関数化しました。

残るは、再帰関数の定義です。しかしこれは、単純に束縛された関数を関数内から呼び出すだけです。
以下のフィボナッチ関数を束縛した `fib` 変数は、関数内部で `fib` を呼び出しています:
...
そのため、関数の再帰呼び出しが多数発生するとエラーが発生します。
